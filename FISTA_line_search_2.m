%% 使用的FISTA的第二种线搜索方法和等步长梯度下降法并比较结果
% 参数说明：
...
% 调用FISTA_linreasch_2方法
[K, x_min, f_min, X, F, dF_norm] = FISTA_linreasch_2_method([-10; -1], 1e-3,A,b,0.33);
disp(['利用FISTA_linreasch_2方法: 迭代次数 = ', num2str(K), ', 最优值点 x1 = ', num2str(x_min(1)), ', x2 = ', num2str(x_min(2)), ', 最小值 = ', num2str(f_min)]);
...
% FISTA_linreasch_2 方法
function [K, x_min, f_min, X, F, dF_norm] = FISTA_linreasch_2_method(start, eps,A,b,rho)
    ...
    while dF_norm(k) >= eps && k < max_iter
        ...
        [alpha,X(:, k + 1),r] = line_search_2(alpha,rho,A,b,alpha_0,r,v,X(:, k));
...
function [alpha_new,X,r] = line_search_2(alpha_old,rho,A,b,alpha_0,r_old,v,X_old)
    alpha_new = alpha_0;
    while true
        r = ( - alpha_new*r_old^2 + sqrt( (alpha_new*r_old^2)^2 + 4*alpha_new*alpha_old*r_old^2 ) )/( 2*alpha_old );
        Y = (1 - r) * X_old + r * v;
        X = Y - alpha_new * df(Y,A,b);
        if f(X,A,b) > f(Y,A,b) + (X-Y)'*df(Y,A,b) + norm(X-Y,2)^2/(2*alpha_new)
            alpha_new = alpha_new * rho;
        else
            break
        end
    end
